...
**Added**
- `apps/mobile/modules/instagram-reels/`: a local Expo module exposing `shareToReels({ videoUri, appId, stickerImageUri? })`, which resolves a `Result`-shaped value and never throws. iOS writes Meta's three documented pasteboard keys (`com.instagram.sharedSticker.backgroundVideo`, `...stickerImage`, `...appID`) with a five-minute expiry and opens `instagram-reels://share`; note that on the Reels path the App ID travels on the PASTEBOARD, not as the `source_application` URL parameter the Stories path uses, and passing it the Stories way is a silent no-op. Android sends an explicit `com.instagram.share.ADD_TO_REEL` intent at `com.instagram.android` with `Intent.EXTRA_STREAM`, the `com.instagram.platform.extra.APPLICATION_ID` extra, and a read-URI-permission grant made BOTH by flag-plus-ClipData and by an explicit `grantUriPermission()` call. The clip is served from the module's own `ReelsFileProvider` rather than another package's.
...
- Tests: `__tests__/modules/instagram-reels.test.ts` (26, the JS boundary), `__tests__/modules/instagram-reels-contract.test.ts` (19, unreachability plus the cross-language string contracts no compiler checks), `__tests__/plugins/with-instagram-reels.test.ts` (14). Mobile 872 -> 931, web 90 and shared 246 unchanged, all green.

**Why each design decision, where it is not obvious**
- **Meta's media limits live in TypeScript and travel to native on every call.** Three copies of "3 to 60 seconds, under 50 MB" in a Swift file, a Kotlin file and a TypeScript file is three chances for one platform to enforce a different window and for nobody to notice until a user hits it. Duration is read from the file itself on the native side, not trusted from the caller, because the caller's idea of the length comes from the render request and the clip is what Instagram measures. A clip under 3s is a hard Instagram rejection; the 50 MB ceiling is Meta's recommendation, enforced here rather than letting Instagram silently drop the payload.
- **Failure is always named, never silent.** The documented failure mode of this entire mechanism is a no-op: Instagram opens to an empty composer, or does not open, with no error anywhere. Both native halves reject with one of eight explicit `ERR_REELS_*` codes (passed, not inferred from a class name, because Swift and Kotlin infer by different rules), which the JS wrapper maps to a stable failure value plus user-facing copy. The raw native text travels separately in `detail`, for logs, so a user is never shown an error code.
...
- **The Android URI grant is doubled on purpose.** `FLAG_GRANT_READ_URI_PERMISSION` covers an intent's `data` URI and its `ClipData`. `EXTRA_STREAM` is migrated into `ClipData` only for `ACTION_SEND` / `ACTION_SEND_MULTIPLE`, and this is a custom action, so nothing migrates. Without both the `ClipData` attachment and the explicit `grantUriPermission()`, Instagram receives a URI it cannot open and the handoff presents as "Instagram ignored us".
- **This replaces the user's clipboard on iOS, and there is no way around it.** The handoff IS a pasteboard write. The items carry an expiry and are cleared again if the composer does not open, but whatever was copied is gone. The availability check in jits-s6mi.3 should therefore run before this, so a user without Instagram never pays that cost.
- **No Android sticker overlay.** The iOS sticker pasteboard key is documented; the equivalent Android extra for the Reels action is not, and inventing one would be an unverifiable guess whose failure mode is a silent drop. `stickerImageUri` is accepted and ignored there.

**Verified, and not verified**
- Verified on a dev box: `expo-modules-autolinking search` discovers the module for both `apple` and `android` with the right Swift class, Kotlin FQN and podspec name; a real `expo config --type introspect` run shows `LSApplicationQueriesSchemes: ["instagram-reels", "instagram"]` and a `<queries>` block carrying `com.instagram.android` APPENDED beside the browsable-intent query Expo already generates; `npx expo export --platform ios --no-bytecode` bundles the JS half; typecheck, the full suite and `build:web` are green; `apps/web` lint holds at its 3 pre-existing errors.
- NOT verified, and not verifiable without a physical device: that Instagram reads the pasteboard item, that the composer opens with the clip in it, that `grantUriPermission` makes the FileProvider URI readable to Instagram, that the Swift and Kotlin compile at all (no gate here builds native code, and `expo export` does not touch it), and how often iOS 16's cross-app pasteboard prompt appears or what it costs in conversion. The JS tests prove the wrapper's contract and degradation; they prove nothing about the handoff. On-device verification is jits-s6mi.6, and it additionally needs a registered Facebook App ID in Live mode (jits-s6mi.8), without which this path cannot work at all.
