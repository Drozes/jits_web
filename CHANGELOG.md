...
- Tests: `__tests__/modules/instagram-reels.test.ts` (34, the JS boundary), `__tests__/modules/instagram-reels-contract.test.ts` (29, the unreachability guard plus the cross-language string contracts no compiler checks), `__tests__/plugins/with-instagram-reels.test.ts` (14). Mobile 872 -> 949, web 90 and shared 246 unchanged, all green.
...
- **The user's clipboard is restored, not destroyed.** The handoff IS a pasteboard write; there is no way to hand the composer a video without replacing `UIPasteboard.general`. The previous contents are now snapshotted and put back when the composer does not open, so someone without Instagram no longer pays for our failed handoff with whatever they had copied. The snapshot is guarded on `numberOfItems > 0`, which does not trigger the iOS 16 cross-app paste prompt, so an empty pasteboard costs nothing; on a non-empty one the read itself can prompt, which is a real trade and a device-test question.
