...
- `getGymDetail`'s ten reads are sorted into fatal and degraded. Fatal (the Result refuses the payload): the gym row, the session list, and the two capability booleans `isMemberGym` / `isGymManager`, since a false `isGymManager` silently removes the Manage entry from a real manager. Degraded (logged, defaulted, payload still returned): participant and RSVP counts, creator names, the athlete's own rsvp/checked-in lists, and the member count. Promoting those to fatal would hide a LIVE session behind an error plate, which is the same harm by another route.
