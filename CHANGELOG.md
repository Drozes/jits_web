...
- Home "Resume your match" card (jits-jitg, mirrors mobile's): new `apps/web/components/domain/resume-match-card.tsx`, fetched server-side on Home with `getMyActiveMatch`, offering an open sessionless match (in progress within 60 min, or pending within 10 min) by a tap, never navigating on its own; while it shows it is Home's one Signal Red CTA (the live session plate's Enter Lobby steps down to secondary). Pending matches the athlete navigated away from are not offered again, via a per-athlete session cookie (new `apps/web/lib/arena/left-matches.ts`); a closed or reloaded tab is not leaving. This also covers a tab closed with a Join toast unanswered.
...
- `tools/match-loop/scenarios/extended/e19-many-on-one.ts` (E19): Red and Green both challenge Blue; Blue accepts the prompt on screen (which one is read from the DB). Oracles: the accepted challenger lands in the one match, `db:other-challenge-declined`, `bot:other-challenger-received-declined` (the broadcast), `bot:other-challenger-plate-clears`, `ui:no-second-prompt-mid-match` (sampled in the background from the Accept tap until 3 s into the weight step; a prompt that reappears after the accepted one closed fails it), `ui:no-error-toast`, `ui:no-info-toast`, `ui:no-prompt-after-match`.
...
- Review follow-up: the no-toast oracles can no longer pass vacuously. `toastPositiveControl` (flows.ts) runs once per run in the first of E18 to E20: Blue challenges Red, Red declines, and `ui:toast-visible` must capture the info toast "Demo Red declined.". `ui:no-error-toast` / `ui:no-info-toast` are hard only after that control passed in the same run, informational otherwise (`toastOracleMode`), and skipped with zero screen samples. E18 and E19 re-count after the accepted-fallback window (`db:still-one-match-after-fallback-window`, accept + 14 s). E19's mid-match check is a background `PromptSampler` (`secondPromptSeen`). E20 pauses the toast watch while the fallback timing is measured and records `fallback_start_observed_ms` in the trace. `waitOutgoingOutcome`'s safety net fires at most once: a failed re-read is traced (`accepted_fallback_read_failed`) and never re-armed, mirroring `recheckOutgoing`. `LOOP.md` extended-tier runtime estimate updated.
...
- `tools/match-loop/sim/match-detail.ts`: page objects for the history entry points, the detail marker, Watch buttons and the player state marker. Every tap target is scrolled into the band between the header and the tab bar first (a row under the tab bar taps a tab), tabs are proven by a tab-only element, the Home "Me" scope is proven by its "View all" link, and the athlete entry is reached independently (Stats, detail, opponent row).
- E17 upload-cap hygiene: the backend caps uploads at 10 per athlete per rolling 24h from the append-only `public.video_upload_events` ledger (a row DELETE does not refund), so E17 deletes its own ledger rows through the local psql at cleanup, checks the remaining budget before seeding, and maps a P0001 `upload_rate_limited` refusal to an `env_error` with the reason. `removeSeededVideo` verifies the row delete returned exactly one row. Documented in `tools/match-loop/LOOP.md` (known harness facts).
