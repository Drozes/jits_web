...
- **Mobile Arena concurrency (`lib/arena/use-arena-challenge.ts`):** the challenger enters a match only on `started` (never on `accepted`, the accepter is creating the match then), with a 12s `ACCEPTED_FALLBACK_MS` safety net that starts it itself; entering a match declines every other fresh pending incoming challenge; crossing challenges (A and B challenge each other) tie-break on the lower challenge id as canonical so both land in ONE match. The roster re-reads when a `lobby:online` id is missing from it (`use-roster-lobby-sync.ts`). Web has only partial parity (ignores `accepted`, joins on `started`, but no tie-break, fallback or decline-on-entry: jits-7dqt).
- **Known gap, do not assume it works:** web mounts the handshake app-wide too (`apps/web/components/arena/arena-bootstrap.tsx`), and its `recover()` in `apps/web/hooks/use-arena-challenge.ts` restores the athlete's own fresh outgoing challenge at mount / go-live / tab-visible, but it still never surfaces an already-pending INCOMING one: on web a challenge that arrived while the recipient was not live (or the tab was closed) is invisible to them. On both platforms a pending challenge older than the live-prompt window (`ARENA_CHALLENGE_FRESH_MS`, 10 min) is never surfaced. `challenges.expires_at` still defaults to 7 days against a cap of 3 pending; the frontend mitigates it (jits-celf) by withdrawing the athlete's OWN outgoing pending challenges older than that window on mount / go-live / foreground and on a capped insert (retry once), via `cancelStaleOutgoingChallenges()`. That also withdraws web profile-sheet challenges after 10 minutes (their view/accept pages are hidden anyway). The backend sweep is still a separate bead.
...
- **Video:** web records WebM via `MediaRecorder`; mobile records MP4 via `expo-camera`, streamed via `FileSystem.uploadAsync` (BINARY_CONTENT, no base64) to the `match-videos` bucket. The key is built by `buildMatchVideoStoragePath()` in `packages/shared/src/api/mutations.ts` and is `<match_id>/<uploader_athlete_id>/<unix_ts>.<ext>` (no `matches/` prefix). Do not hand-roll it.
...
- **TestFlight build:** the `testflight-release` skill (`eas build -p ios --profile production --auto-submit`). A new native binary, required for anything OTA cannot carry. **A FINISHED EAS build is not in TestFlight until it is submitted**: confirm the submission (`eas build:view <id> --json`) before publishing an OTA for its runtime. On 2026-09-26 build 22 (v0.3.0) had never been submitted, so the 0.3.0 OTA reached nobody while testers sat on 0.2.0. A new `expo.version` also needs Beta App Review before external testers see it.
...
### Production environment facts (verified 2026-09-26, re-verify before relying on them)
- **Web prod URL is `https://jitsweb.vercel.app`** (Vercel team `matthew-sponagles-projects`, project `jits_web`, deploys from `main`). **`elorated.com` is NOT attached to Vercel**: it is registered and hosted at GoDaddy (site-builder page), so `buildShareUrl` links (`https://elorated.com/athlete/...`), universal links and the AASA file all 404 until the domain moves (jits-x1t2). Confirm the owning account before changing DNS.
- **Supabase prod (`rzowigdcdojlovlwwpkx`):** email sign-up needs no confirmation (`mailer_autoconfirm: true`); Google on; **Apple off** (the web login's Apple button errors, jits-2cb2). Redirect allow-list, Google client IDs and SMTP are unverified (jits-wn5g).
- **Push never fires on prod** (jits-qokf): `notify_push_function` posts to `current_setting('app.settings.edge_function_url')`, unset on hosted Supabase, so it falls back to `host.docker.internal`; VAPID secrets are missing too. Fix via Vault, not a DB GUC.
- **Edge functions on prod are from 2026-05-28.** The repo's `video-analyze-chunk` / `video-merge-analysis` need secret `SLICER_SHARED_SECRET`, which prod lacks: set it before redeploying them or video analysis breaks.
- **Video uploads:** enabled for every athlete via the auto-allowlist trigger; cap 10 per athlete per rolling 24h (function default, GUC unset; deletes do not refund).
