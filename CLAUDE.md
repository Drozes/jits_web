...
- Active route set is the filesystem under `apps/web/app/` and `apps/mobile/app/`. **Both navs are now the same four tabs: Home, Arena, Rankings, Profile.** Mobile's Gyms and Gym-manager tabs were removed from `(tabs)/` and their routes moved to a stack under `apps/mobile/app/(app)/gyms/` and `(app)/gym-manager/`; they are still live and still reachable by link, exactly like web's `/gyms`.
...
- **Known gap, do not assume it works:** neither platform fetches already-pending challenges at mount, so a challenge that arrives while the recipient is not on the Arena is currently invisible to them. `getPendingChallengesForAthlete()` exists in `@jits/shared/api/queries` for exactly this and is not yet wired into either UI. `challenges.expires_at` defaults to 7 days against a cap of 3 pending, so stranded challenges lock a challenger out.
